...
(110, 'Thunderbolt Cable', 'ACC-505-Z', 'Aksesoris', 450000, 100, 'B-03', 'Lulus');
 ...
 SELECT DISTINCT kategori FROM inventaris_gudang_pusat WHERE kode_sku LIKE '%ELEC%' AND kode_sku LIKE '%X' AND harga_satuan > 5000000;
 
 SELECT nama_barang, stok,
 CASE
 WHEN stok = 0 && kategori = 'aksesoris' THEN 'urgent-Restock Segera'
 WHEN stok < 10 && stok <> 0 && kategori IN ('laptop','smartphone') THEN 'High Priority'
 WHEN stok BETWEEN 10 AND 50 THEN 'medium'
 ELSE 'aman'
 END prioritas_restock
 FROM inventaris_gudang_pusat
 ORDER BY stok ASC;
 
 SELECT * FROM inventaris_gudang_pusat WHERE lokasi_rak LIKE 'B%' && harga_satuan NOT BETWEEN 1000000 AND 5000000 && kategori <> 'monitor';
